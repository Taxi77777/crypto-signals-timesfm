    ...
    for sig_tuple, df_15m, df_30m in mtf_results:
        ...
        try:
            ...
            send_message(
                ...
                f"📍 Retest Zone       : *{'Double Bottom (Bas)' if target_signal == 'BUY' else 'Double Top (Haut)'}*\n"
                f"⏱️ Timing Oscillator : `{bary_timing:+.2f}` (Extrême Validé)\n"
                f"🕯️ Mèche de Rejet    : *Physique ≥ 15% Confirmée*\n"
                f"🕒 *Heure 1er Rejet*   : *Bougie de {datetime.now(PARIS_TZ).strftime('%H:%M')} (Heure de Paris)*\n"
