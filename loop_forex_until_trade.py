...
def main():
    logger.info("🚀 Démarrage de la boucle de scan continu Forex (IA & Mostafa Belkhayate)...")
    scan_count = 0
    ...
    while True:
        ...
        try:
            ...
            if signals and len(signals) > 0:
                logger.info(f"🎉 {len(signals)} SIGNAL/SIGNAUX FOREX CONFIRMÉS TROUVÉS !")
                for s in signals:
                    logger.info(f"👉 {s['symbol']} {s['direction']} | Entrée: {s['entry_price']} | TP: {s['tp']} | SL: {s['sl']}")
