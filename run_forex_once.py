...
===============================================================================
- Multi-Timeframe : 5m, 15m, 30m, 1H (Scrute les 4 Timeframes)
...
def run_forex_scan():
    logger.info("==================================================")
    logger.info("DEMARRAGE SCANNER FOREX MULTI-TIMEFRAME (5m, 15m, 30m, 1h)")
    logger.info("==================================================")
    ...
    for tf in TIMEFRAMES:
        ...
        for symbol in FOREX_PAIRS:
            ...
            try:
                ...
                df = prepare_forex_indicators(df)
                cur_price = float(df["close"].iloc[-1])

                # ── 1. VERIFICATION METHODE BELKHAYATE & MECHES DE REJET ──
                lower_wick = float(df["lower_wick_pct"].iloc[-1])
                upper_wick = float(df["upper_wick_pct"].iloc[-1])
                timing     = float(df["belkhayate_timing"].iloc[-1])
                low_zone   = float(df["belkhayate_lower_zone"].iloc[-1])
                up_zone    = float(df["belkhayate_upper_zone"].iloc[-1])

                has_buy_wick  = (lower_wick >= 0.15)
                has_sell_wick = (upper_wick >= 0.15)

                # ── 2. DETECTER CROISEMENT PRECIS MA 30 / MA 60 (0 à 1 bougie max) ──
                ma30_curr = float(df["ma30"].iloc[-1])
                ma60_curr = float(df["ma60"].iloc[-1])
                ma30_p1   = float(df["ma30"].iloc[-2])
                ma60_p1   = float(df["ma60"].iloc[-2])
                ma30_p2   = float(df["ma30"].iloc[-3])
                ma60_p2   = float(df["ma60"].iloc[-3])
                ...
                belkhayate_buy  = (cur_price <= low_zone or timing <= -1.5 or is_exact_golden_cross) and has_buy_wick
                belkhayate_sell = (cur_price >= up_zone  or timing >= 1.5  or is_exact_death_cross)  and has_sell_wick
                ...
                if ai_enabled:
                    tfm_preds = predict_timesfm(df["close"].values)
                    cho_preds = predict_chronos(df["close"].values)
                    ...
                    if tfm_dir != direction and cho_dir != direction and tfm_dir != "HOLD" and cho_dir != "HOLD":
                        logger.info(f"[{tf}] {clean_pair} : Desaccord IA ({tfm_dir}/{cho_dir} vs Belkhayate {direction}) — ignore.")
                        continue
                ...
                logger.info(f"SIGNAL FOREX [{tf}] CONFIRME BELKHAYATE — {clean_pair} {direction} | Prix: {cur_price:.5f} | TP: {tp_price:.5f} | SL: {sl_price:.5f} | Timing: {timing:.2f}")
    ...
    logger.info(f"=== Scan Forex Multi-Timeframe termine. {len(signals_found)} signal/signaux retenus ===")
